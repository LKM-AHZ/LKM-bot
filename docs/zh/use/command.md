...
LKMBot 的指令通过插件机制注册。为了保持主程序轻量，当前只有少量基础指令随 LKMBot 主程序内置加载；更多管理类、扩展类指令已经迁移到独立插件中维护。
...
以下指令由 LKMBot 主程序自带，默认随 LKMBot 加载：

- `/help`：查看当前启用的指令和 LKMBot 版本信息。
...
- `/dashboard_update`：更新 LKMBot WebUI。该指令需要管理员权限。
...
- `UID`：当前发送者的用户 ID。它通常用于添加 LKMBot 管理员。
...
原始 UMO 往往比较长，也不一定能直接看出它对应哪个群或哪个用户。设置 `/name` 后，LKMBot 会在 WebUI 的 UMO 列表、会话来源选择、定时任务投递目标、对话数据等位置优先展示这个别名，帮助管理员更快识别和选择目标会话，降低配置路由规则、Cron 投递目标或会话规则时选错来源的概率。
...
- 私聊中默认普通用户可使用。
- 群聊默认「跟随对话隔离」：开启「隔离对话」且渠道实际完成隔离时，所有人可使用；关闭时仅 LKMBot 管理员可使用。管理员指配置的管理员 ID，不是自动识别的群管理员。不支持隔离的渠道仍按共享群聊限制权限。
...
如果当前会话没有正在运行的任务，LKMBot 会提示当前会话没有运行中的任务。
...
如果当前不在任何对话中，LKMBot 会提示先使用 `/new` 创建对话。
...
部分指令需要 LKMBot 管理员权限，例如 `/dashboard_update`、`/name`、`/op`、`/deop`、`/provider`、`/model`、`/persona` 等。
