...
Starting from version `v3.4.2`, LKMBot supports a code interpreter to enhance LLM capabilities and enable various automated operations.
...
## Running LKMBot with Docker on Linux

If you've deployed LKMBot using Docker, some additional setup is required.

1. When starting the Docker container, mount `/var/run/docker.sock` inside the container. This allows LKMBot to launch sandbox containers.
...
2. Use the `/pi absdir <absolute-path>` command during chat to set the absolute path of LKMBot's data directory on your host machine.

Example:

![image](https://files.astrbot.app/docs/source/images/code-interpreter/image-4.png)

## Running LKMBot from Source on Linux

**If your Docker commands require sudo privileges**, you need to start LKMBot with `sudo`, otherwise the code interpreter won't be able to invoke Docker due to insufficient permissions.
...
When the code interpreter is triggered for the first time, LKMBot will automatically pull the image, which may take some time. Please be patient.
