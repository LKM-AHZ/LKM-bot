...
Agent 执行器是 LKMBot 中用于执行 Agent 的组件，Agent 执行器负责了所有关于 AI 的功能。

LKMBot 内置了**强大的** Agent 执行器，用户也可以选择接入第三方的 Agent 执行器服务，例如 Dify、Coze、阿里云百炼应用、DeerFlow 等，或者自己开发 Agent 执行器。
