...
LKMBot 的 HTTP 代理在 WebUI：`设置 → 网络 → 代理与依赖源 → HTTP 代理`。填写的地址由 **LKMBot 容器内部**访问，因此 `http://127.0.0.1:7890` 会连到 LKMBot 容器自己，而不是宿主机或其他容器。
...
LKMBot 与代理在同一 Docker 网络时，用容器名，例如 `http://clash:7890`。
