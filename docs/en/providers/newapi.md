...
LKMBot can integrate with NewAPI as a model provider, so you can access those model services through LKMBot.
...
### 2. Configure NewAPI in LKMBot

Open **Providers → Chat Completion** in the LKMBot WebUI and click **Add**.
...
You have now successfully configured NewAPI as an LKMBot model provider.
