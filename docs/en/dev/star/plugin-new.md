...
# LKMBot Plugin Development Guide 🌠

Welcome to the LKMBot Plugin Development Guide! This section will guide you through developing LKMBot plugins. Before we begin, we hope you have the following foundational knowledge:
...
1. Open the LKMBot plugin template: [helloworld](https://github.com/Soulter/helloworld)
...
Clone both the LKMBot main project and the plugin repository you just created to your local machine.
...
Then, use `VSCode` to open the `LKMBot` project. Navigate to the `data/plugins/<your-plugin-name>` directory.

Update the `metadata.yaml` file with your plugin's metadata information.

> [!WARNING]
> Please make sure to modify this file, as LKMBot relies on the `metadata.yaml` file to recognize plugin metadata.
...
Plugins can provide a `skills/` directory. After LKMBot loads the plugin, valid Skills inside that directory are automatically included in the Skill Manager, with their source shown as the plugin.
...
### Declare LKMBot Version Range (Optional)

You can add an `astrbot_version` field in `metadata.yaml` to declare the required LKMBot version range for your plugin. The format follows dependency specifiers in `pyproject.toml` (PEP 440), and must not include a `v` prefix.
...
If the current LKMBot version does not satisfy this range, the plugin will be blocked from loading with a compatibility error.
In the WebUI installation flow, you can choose to "Ignore Warning and Install" to bypass this check.

### Debugging Plugins

LKMBot uses a runtime plugin injection mechanism. Therefore, when debugging plugins, you need to start the LKMBot main application.

You can use LKMBot's hot reload feature to streamline the development process.

After modifying the plugin code, open `Extensions → Plugins` in the LKMBot WebUI, find your plugin, and click the refresh icon (`Reload Extension`) on its card.
...
Currently, LKMBot manages plugin dependencies using pip's built-in `requirements.txt` file. If your plugin requires third-party libraries, please be sure to create a `requirements.txt` file in the plugin directory and list the dependencies used, to prevent Module Not Found errors when users install your plugin.
...
Thank you for contributing to the LKMBot ecosystem. Please follow these principles when developing plugins, which are also good programming practices:
