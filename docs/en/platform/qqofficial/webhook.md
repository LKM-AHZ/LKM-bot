...
5. Configure DNS and a reverse proxy for your server so HTTPS requests are forwarded to LKMBot's `6185` port.
6. Go back to the QQ Open Platform bot management page, open `Development -> Callback Configuration`, and enter the Webhook callback URL generated by LKMBot.
...
8. Save the callback configuration, then restart LKMBot.

> [!TIP]
> With `Unified Webhook Mode`, LKMBot generates a unique Webhook callback URL automatically. You can find it under `Data & Logs → Logs`, or select the bot in `Platforms` and click `View Webhook URL`.
...
Webhook mode also requires selecting the group event `GROUP_MESSAGE_CREATE` in QQ Open Platform callback configuration. Otherwise, LKMBot cannot receive full group message events.
...
After saving, configure DNS and reverse proxy for your server. Forward requests to LKMBot's `6185` port. If `Unified Webhook Mode` is disabled, forward requests to the port configured in the previous step instead.
...
After you save the bot in LKMBot, LKMBot generates a unique Webhook callback URL. You can find it under `Data & Logs → Logs`, or select the bot in `Platforms` and click `View Webhook URL`.
