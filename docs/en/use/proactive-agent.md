...
LKMBot introduces a Proactive Agent system, enabling LKMBot to not only respond passively to users but also schedule future tasks and proactively execute them at specified times, delivering results (text, images, files, etc.) to users.
...
- **Self-Wakeup**: LKMBot automatically wakes up at the scheduled time to execute tasks.
- **Task Feedback**: After execution, LKMBot reports the results back to the task creator.
...
You can view and manage all future tasks by expanding **More Features** in the left navigation bar and clicking **Future Tasks** (`/cron`) of the LKMBot WebUI.

![](https://files.astrbot.app/docs/source/images/proactive-agent/image-1.png)

### Supported Platforms

Scheduling tasks is supported on all platforms. However, due to some platforms not providing APIs for proactive message pushing, only the following platforms support LKMBot proactively pushing results to users:
- Telegram
- OneBot (QQ)
- Slack
- Feishu (Lark)
...
To make it easier for Agents to send images, audio, video, and other files directly to users, LKMBot provides a `send_message_to_user` tool by default.
