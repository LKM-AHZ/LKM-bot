...
LKMBot 支持接入微信公众平台（版本 >= v3.5.8），并以微信公众号的形式接入，届时，您可以直接在微信公众号聊天界面中与 LKMBot 进行交互。
...
记录开发者 ID(AppID) 和开发者密码(AppSecret)，分别填入 LKMBot 配置的 `appid` 和 `secret` 处。
...
`Token` 由自己填写，请随意填写一个字符串，长度 3-32 位。并同样填入 LKMBot 配置的 `token` 处（一定要相同）。

`EncodingAESKey` 请点击随机生成，然后复制到 LKMBot 配置的 `encoding_aes_key` 处。

建议保持 `统一 Webhook 模式 (unified_webhook_mode)` 为开启状态。

现在应该已经填完 LKMBot 连接到微信公众平台的所有配置项。点击创建弹窗中的 `保存`（修改已有机器人时点击 `保存更改`），等待适配器重新加载。
...
> 注意⚠️：仅支持 80 或者 443 端口。您可能需要购买域名，然后反向代理流量到 LKMBot 所在服务器的 `6185` 端口（如果开启了统一 Webhook 模式）或 `6194` 端口（如果没有开启统一 Webhook 模式），或者将端口改成 80 端口（注意服务器需要没有软件在占用 80 端口）。
...
点击左下角你的账号头像，点击账号详情，找到 `二维码`，扫码进入到公众号聊天界面，发送 `help`，看看 LKMBot 是否能够回复。
