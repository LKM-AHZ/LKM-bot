...
> [!TIP]
> server-satori 是 Koishi 平台的一个插件，可以将 Koishi 作为 Satori 协议的服务端，让 LKMBot 通过 Satori 协议接入 koishi 响应消息。
...
此时，你的 LKMBot 应该已经通过 Satori 协议成功连接到了 Koishi 的 server-satori 插件。

在 Koishi 的沙盒里 向机器人发送 LKMBot的指令（例如：`/help`）进行测试，
