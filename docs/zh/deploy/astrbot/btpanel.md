...
LKMBot 已经上架至宝塔的 Docker 应用商店，支持一键安装。
...
进入 Docker 的应用商店，搜索 `LKMBot`，如下图所示。

![image](https://files.astrbot.app/docs/source/images/btpanel/image.png)

点击安装，等待安装成功。

安装成功后，点击左侧 `安全`，放行对应的 LKMBot 端口（默认是 6185 端口）。

如果您正在使用 AWS、阿里云、腾讯云等厂商的云服务器，请确保其安全组也放行了对应的端口。

## 访问 LKMBot

访问 `http://IP:6185` 即可访问 LKMBot 的管理面板。

> [!TIP]
> 默认情况下，上述方法只会放行一个 6185 端口。如果需要部署消息平台，需要额外放行对应的端口。点击上栏 `容器`，找到 LKMBot 容器，点击 `管理`，点击 `编辑容器`，添加对应的端口即可。
>
