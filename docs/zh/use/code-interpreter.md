...
在 `v3.4.2` 版本及之后，LKMBot 支持代码执行器以强化 LLM 的能力，并实现一些自动化的操作。
...
## Linux Docker 启动 LKMBot

如果您使用 Docker 部署了 LKMBot，需要多做一些工作。

1. 您需要在启动 Docker 容器时，请将 `/var/run/docker.sock` 挂载到容器内部。这样 LKMBot 才能够启动沙箱容器。

```bash
sudo docker run -itd -p 6180-6200:6180-6200 -p 11451:11451 -v $PWD/data:/LKMBot/data -v /var/run/docker.sock:/var/run/docker.sock --name lkmbot lkmbot:latest
```

2. 在聊天时使用 `/pi absdir <绝对路径地址>` 设置您宿主机上 LKMBot 的 data 目录的所在目录的绝对路径。

例子：

![image](https://files.astrbot.app/docs/source/images/code-interpreter/image-4.png)

## Linux 手动源码 启动 LKMBot

**如果你的 Docker 指令需要 sudo 权限来执行**，那么你需要在启动 LKMBot 时，使用 `sudo` 来启动，否则代码执行器会因为权限不足而无法调用 Docker。
...
在第一次触发代码执行器时，LKMBot 会自动拉取镜像，这可能需要一些时间。请耐心等待。
