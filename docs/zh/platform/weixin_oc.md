...
LKMBot 支持通过 `个人微信` 适配器接入微信个人号。该适配器基于**腾讯微信官方** `openclaw-weixin` 接口实现，使用扫码登录和长轮询收发消息，不需要配置 Webhook 回调地址。
...
> [!TIP]
> `token` 和 `account_id` 会在扫码登录成功后由 LKMBot 自动保存，通常不需要手动填写。

## 扫码登录

选择 `个人微信` 后，LKMBot 会自动向微信接口申请登录二维码，并直接显示在创建机器人弹窗中。使用手机微信扫码确认后，二维码会显示登录成功状态，此时点击 `保存` 即可完成创建。

登录成功并保存后，LKMBot 会自动保存登录态。后续重启时，如果登录态仍有效，通常不需要再次扫码。
...
登录成功后，用微信发送一条消息。如果 LKMBot 能正常回复，说明接入成功。
...
接收到的图片、视频、文件、语音会被 LKMBot 下载并解密后保存到本地临时目录：

`data/temp`

这些文件属于 LKMBot 的临时缓存文件，后续插件、Agent 或文件服务可以继续读取和处理。
