...
The Mattermost adapter connects to your Mattermost server through a Bot Token and WebSocket. After finishing the two parts below, LKMBot can send and receive messages in Mattermost channels and direct messages.

## Create the LKMBot Mattermost Platform Adapter
...
- `Mattermost Reconnect Delay`: how long LKMBot waits before reconnecting after a WebSocket disconnect, default `5`
...
After creation, copy the generated Bot Token. It is shown only once. Paste it into LKMBot's `Mattermost Bot Token` field.

### 3. Add the Bot to a Channel

Add the bot to the channel where LKMBot should work. Otherwise the bot will not be able to properly receive and send messages in that channel.
...
If both LKMBot and Mattermost run in containers, prefer an address reachable from the LKMBot container, such as the Mattermost service name on the same Docker network.

## Start and Verify

After saving the LKMBot platform adapter configuration:

1. Make sure the LKMBot logs do not show Mattermost authentication or WebSocket connection errors.
2. Send a message in a channel that includes the bot, or send the bot a direct message.
3. If LKMBot replies normally, the integration is working.
...
- `Mattermost URL` points to an address LKMBot can actually reach
- Your Mattermost reverse proxy forwards WebSocket traffic correctly

### Mattermost Opens in Browser but LKMBot Still Cannot Connect

If LKMBot runs in a container while `Mattermost URL` is set to `localhost` or `127.0.0.1`, LKMBot will connect to itself instead of the Mattermost service. In that case, switch to an address reachable inside the Docker network.
