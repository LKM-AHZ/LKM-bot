...
欢迎来到 LKMBot 插件开发指南！本章节将引导您如何开发 LKMBot 插件。在我们开始之前，希望你能具备以下基础知识：
...
> [!WARNING]
> 请务必修改此文件，LKMBot 识别插件元数据依赖于 `metadata.yaml` 文件。
...
您可以使用 LKMBot 的热重载功能简化开发流程。

插件的代码修改后，在 LKMBot WebUI 的 `插件` 页找到自己的插件，点击插件卡片上的刷新图标（`重载插件`）。
